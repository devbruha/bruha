<?php  
class nowtorontocomedy_scraper extends CI_Model {  
    //include_once('DOM/simple_html_dom.php');
    function nowtorontocomedy_scraper() 
    {  
        parent::__construct(); 
    ...
    function nowtorontocomedy_main(){
        $url_base = 'http://www.nowtoronto.com/stage/listings/';
        $url_comedy = 'http://www.nowtoronto.com/stage/listings/index.cfm?subsection=comedy';
        //echo "running this";
        $html = file_get_html($url_comedy);
        $fp = fopen('nowtoronto_comedy_data.csv', 'w');
        ...
        foreach($html->find('div[class=listing-readmore]') as $result) {
            ...
            $url2=$url_base.$this->parse_location($result);
            ...
            foreach($html2->find('div[id=leftColFrame]') as $result2)
            {
                ...
                $latitude = $latlong[0];
                $longitude = $latlong[1];
                //$active_days=parse_active_days($result2->find('script[type=text/javascript]',0)->innertext);
                //echo $active_days."<br/>";
                echo "<br/>";
                /*$latlong = geocode->setAddress(urlencode($street_address));
                $latitude = $latlong[0];
                $longitude = $latlong[1];
        //        echo "$street_address";
