...
function addToList(allMarkers,i){
    ...
    eventContent += '<a onclick="return ajaxLoadEvent('+allMarkers.event_id+');" class="yellowButton viewOnMap" href="' + baseUrl + 'index.php/events/view/'+allMarkers.event_id+'">VIEW EVENT</a>';
    eventContent += '</div>';
    ...
    $('#advancedSearchForm').submit(function(){
        ...
        geocoder.geocode( { 'address': address}, function(results, status) {
            if (status != google.maps.GeocoderStatus.OK && (address != '')) {
                alert('We cannot seem to look up your specific locations. Please check you have spelled it correctly and have included a city, province and county.');
            }else{
                if(address == '' || address == undefined){
                    bounds = map.getBounds();
                    center = bounds.getCenter();
                ...
                preAddMarkers( searchString,startDate,endDate,anyDate,twlelveHour,newLat,newLng,catString,featured,genre);
